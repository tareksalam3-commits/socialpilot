...
function Settings({ data, mutate }: { data: Record<string, unknown>; mutate: (action: () => Promise<unknown>, confirmText?: string) => Promise<void> }) {
  const settings = (data.settings ?? {}) as Record<string, unknown>;
  const flags = ['lead_hunter_enabled', 'lead_search_enabled', 'lead_ai_enabled', 'lead_scoring_enabled', 'lead_export_enabled', 'lead_campaigns_enabled', 'lead_social_sources_enabled'];
  const [searxngUrl, setSearxngUrl] = useState(String(settings.searxng_base_url ?? ''));
  const [searchMode, setSearchMode] = useState(String(settings.default_search_mode ?? 'balanced'));
  const [maxQueries, setMaxQueries] = useState(String(settings.max_queries ?? 24));
  ...
  return <div className="flex flex-col gap-3">
    <Card>
      ...
      <p className="text-ink-500 text-xs leading-6 mb-3">يُحفظ العنوان للاستخدام الخلفي فقط. إذا لم يتم ضبطه سيظهر النظام صراحةً BLOCKED — REQUIRES CONFIGURATION ولن ينشئ نتائج.</p>
    ...
    <Card><h2 className="text-ink-100 font-semibold mb-2">القيم الحالية</h2><div className="grid grid-cols-2 gap-2 text-xs text-ink-400"><span>حد البحث: {String(settings.default_search_limit ?? 'غير معروف')}</span><span>أقصى تشغيل: {String(settings.max_runtime_seconds ?? 'غير معروف')} ثانية</span><span>Max Queries: {String(settings.max_queries ?? 'غير معروف')}</span><span>Max Fetches: {String(settings.max_fetches ?? 'غير معروف')}</span><span>المصادر: {String(settings.max_sources ?? 'غير معروف')}</span><span>الاحتفاظ: {String(settings.default_retention_days ?? 'غير معروف')} يوم</span></div></Card>
  </div>;
}
