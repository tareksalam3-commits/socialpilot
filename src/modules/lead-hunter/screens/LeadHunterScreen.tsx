...
function JobProgress({ job, stats, leads }: { job: LeadSearchJob; stats: LeadSearchStats | null; leads: Lead[] }) {
  ...
  return (
    <div>
      ...
      {stats?.aiProvidersUsed?.length ? <Card className="mb-4"><p className="text-ink-500 text-xs">أدوات الذكاء الاصطناعي المستخدمة</p><p className="text-ink-200 text-sm mt-2" dir="ltr">{stats.aiProvidersUsed.join(' · ')}</p><p className="text-ink-500 text-xs mt-2" dir="ltr">{(stats.aiModelsUsed ?? []).slice(0, 3).join(' · ')}{(stats.aiFallbacks ?? 0) > 0 ? ` — fallback: ${stats.aiFallbacks}` : ''}</p>{stats.aiFallbackLog?.length ? <p className="text-ink-600 text-[11px] mt-2">تم تجاوز: {stats.aiFallbackLog.slice(0, 3).map((item) => `${item.provider}/${item.model}`).join(' · ')}</p> : null}</Card> : null}
      {(stats?.strategiesUsed?.length || stats?.categoriesUsed?.length || stats?.strongQueries?.length || stats?.weakQueries?.length) ? <Card className="mb-4"><p className="text-ink-500 text-xs">ملخص البحث الذكي</p><div className="grid grid-cols-2 gap-2 text-xs text-ink-300 mt-2"><span>الاستراتيجيات: {stats.strategiesUsed?.join(' · ') || 'غير معروف'}</span><span>الفئات: {stats.categoriesUsed?.join(' · ') || 'غير معروف'}</span><span>Relevant: {stats.relevantRate ?? 'غير معروف'}%</span><span>Qualified: {stats.qualifiedRate ?? 'غير معروف'}%</span><span>Duplicates: {stats.duplicateRate ?? 'غير معروف'}%</span><span>Verification: {stats.verificationRate ?? 'غير معروف'}%</span></div><p className="text-ink-500 text-[11px] mt-2">استعلامات قوية: {(stats.strongQueries ?? []).slice(0, 4).join(' · ') || 'غير معروف'}</p><p className="text-ink-500 text-[11px] mt-1">استعلامات ضعيفة: {(stats.weakQueries ?? []).slice(0, 4).join(' · ') || 'لا توجد'}</p></Card> : null}
