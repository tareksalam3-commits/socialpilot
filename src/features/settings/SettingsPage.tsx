...
import { COUNTRIES } from '@/constants/dialects';
  ...
  return (
    <Card title={t('settings.general.title')} description={t('settings.general.description')}>
      <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <Info label={t('settings.workspace.country')} value={workspace?.country ?? 'EG'} />
      </dl>
    </Card>
  );
}
...
function WorkspaceTab() {
  ...
  const [country, setCountry] = useState(workspace?.country ?? 'EG');
  ...
  return (
    <Card title={t('settings.workspace.title')} description={t('settings.workspace.description')}>
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-slate-700 dark:text-slate-300">{t('settings.workspace.country')}</label>
          <select
            value={country}
          ...
          >
            {COUNTRIES.map((c) => (
              <option key={c.code} value={c.code}>
                {t('settings.workspace.countryOptionLabel', { native: c.nativeLabel, english: c.label })}
              </option>
            ))}
          </select>
          {language === 'ar' && <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">{t('settings.workspace.countryDialectHint')}</p>}
        </div>
        <div>
