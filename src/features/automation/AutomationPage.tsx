...
export function AutomationPage() {
  ...
  const handleToggleAutoPublish = async () => {
    ...
    try {
      ...
      push({ title: workspace.auto_publish_enabled ? t('automation.autoPublish.pausedTitle') : t('automation.autoPublish.enabledTitle'), variant: 'success' });
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={t('automation.autoPublish.toggleTitle')}
          >
        ...
        {schedulerStatus ? (
          <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <p className="mt-1 font-medium text-slate-900 dark:text-white">{schedulerStatus.last_run_at ? timeAgo(schedulerStatus.last_run_at, t) : t('automation.backgroundJobs.never')}</p>
            ...
            <div>
              ...
              <p className="mt-1 text-slate-600 dark:text-slate-400">{t('automation.backgroundJobs.persistenceDesc')}</p>
            </div>
          </div>
        ) : (
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('automation.backgroundJobs.unavailable')}</p>
        )}
      </Card>
      ...
      <Card>
        ...
        {queue.length === 0 ? (
          <EmptyState icon={<Send className="h-8 w-8" />} title={t('automation.queue.emptyTitle')} description={t('automation.queue.emptyDescription')} />
        ) : (
          <Table headers={[t('automation.queue.header.status'), t('automation.queue.header.content'), t('automation.queue.header.platforms'), t('automation.queue.header.scheduledFor')]}>
            {queue.map((p) => (
              <TableRow key={p.id}>
                <TableCell><Badge variant={p.status === 'publishing' ? 'info' : 'default'}>{t(statusLabelKey[p.status] ?? p.status)}</Badge></TableCell>
                <TableCell className="max-w-xs truncate">{p.title || p.content || t('automation.noContent')}</TableCell>
      ...
      <Card>
        ...
        {scheduledJobs.length === 0 ? (
          <EmptyState icon={<CalendarClock className="h-8 w-8" />} title={t('automation.scheduledJobs.emptyTitle')} description={t('automation.scheduledJobs.emptyDescription')} />
        ) : (
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          {failedQueue.length > 0 && (
            <Button size="sm" variant="outline" onClick={handleRetryAllFailed} disabled={actioningId === 'all'}>
              <RotateCcw className="h-3.5 w-3.5" /> {t('automation.failedQueue.retryAllFailed')}
            </Button>
          )}
        </div>
        {failedQueue.length === 0 ? (
          <EmptyState icon={<CheckCircle2 className="h-8 w-8" />} title={t('automation.failedQueue.emptyTitle')} description={t('automation.failedQueue.emptyDescription')} />
        ) : (
          <Table headers={[t('automation.failedQueue.header.content'), t('automation.failedQueue.header.platform'), t('automation.failedQueue.header.error'), t('automation.failedQueue.header.retries'), t('automation.failedQueue.header.nextRetry'), t('automation.failedQueue.header.actions')]}>
            {failedQueue.map((target) => (
              <TableRow key={target.id}>
                <TableCell className="max-w-xs truncate">{target.post_title || target.post_content || t('automation.noContent')}</TableCell>
                <TableCell className="text-xs text-slate-500 dark:text-slate-400">{target.platform}</TableCell>
                <TableCell className="max-w-xs truncate text-xs text-rose-600 dark:text-rose-400" title={target.error_message ?? undefined}>{target.error_message ?? '—'}</TableCell>
                <TableCell className="text-xs text-slate-500 dark:text-slate-400">{target.retry_count}/{target.max_retries}</TableCell>
                <TableCell className="text-xs text-slate-500 dark:text-slate-400">{target.next_retry_at ? formatDateTime(target.next_retry_at) : target.retry_count >= target.max_retries ? t('automation.failedQueue.gaveUp') : '—'}</TableCell>
                <TableCell>
                  <button
                  ...
                  >
                    <RotateCcw className="h-3.5 w-3.5" /> {t('automation.failedQueue.retry')}
                  </button>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Card>
        <div className="mb-3 flex items-center gap-2">
          <ListChecks className="h-4 w-4 text-slate-400" />
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">{t('automation.logs.title')}</h2>
        </div>
        {logs.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">{t('automation.logs.empty')}</p>
        ) : (
