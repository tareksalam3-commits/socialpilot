...
export function PlaygroundChat() {
    ...
    if (result.result) {
      ...
      try {
        const saved = await persistGeneratedContent({
          ...
          sourceLabel: 'AI Playground',
          stage: 'generated',
          metadata: {
            playground: {
              conversation_id: convId,
              prompt: userMessage,
              model: result.model ?? settings?.default_model ?? null,
              tokens: result.tokensIn + result.tokensOut,
              response_time_ms: result.responseTimeMs,
            },
          },
  ...
  }, [input, workspace, user, activeId, messages, settings, createConversation, addMessage, generate, push, t]);
