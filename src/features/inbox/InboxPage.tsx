...
export function InboxPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Unified Inbox</h1>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Comments and direct messages from all your connected accounts.</p>
      </div>
      ...
      <div className="flex h-[calc(100vh-14rem)] gap-4">
        ...
        <div className="flex w-72 flex-col rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          <div className="border-b border-slate-200 p-3 dark:border-slate-800">
            <div className="relative">
              ...
              <input type="search" placeholder="Search…" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-lg border border-slate-300 bg-white py-1.5 pl-9 pr-3 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" />
            </div>
          </div>
          <div className="flex gap-2 border-b border-slate-200 p-2 dark:border-slate-800">
            <button onClick={() => setShowArchived(false)} className={`flex-1 rounded-lg px-2 py-1 text-xs font-medium ${!showArchived ? 'bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-white' : 'text-slate-500'}`}>Active</button>
        ...
        <div className="flex flex-1 flex-col rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900">
          {activeId ? (
            <>
              ...
              <div className="border-t border-slate-200 p-3 dark:border-slate-800">
                <div className="flex items-end gap-2">
                  <textarea value={reply} onChange={(e) => setReply(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(); } }} rows={2} placeholder="Type your reply…" className="flex-1 resize-none rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100" />
                  <div className="flex flex-col gap-1">
                    <Button size="sm" variant="outline" onClick={handleAIReply} loading={aiGenerating} title="Generate AI reply"><Sparkles className="h-3.5 w-3.5" /></Button>
            ...
            <div className="flex flex-1 items-center justify-center"><EmptyState icon={<Inbox className="h-10 w-10" />} title="Select a conversation" description="Choose a conversation from the list to view messages." /></div>
          )}
        </div>
      </div>
    </div>
