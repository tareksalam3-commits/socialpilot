...
export function ContentCalendarPage() {
  ...
  const handleDrop = useCallback(async (postId: string, dateStr: string) => {
    ...
    push({ title: 'Post rescheduled', description: `Moved to ${formatDateTime(newDate.toISOString())}`, variant: 'success' });
  }, [update, push]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Drag and drop posts to reschedule them.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-lg border border-slate-200 dark:border-slate-800">
            {(['month', 'week', 'day'] as ViewMode[]).map((m) => (
              <button key={m} onClick={() => setView(m)} className={`px-3 py-1.5 text-xs font-medium capitalize transition ${view === m ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'text-slate-600 dark:text-slate-400'}`}>{m}</button>
            ))}
          </div>
      ...
      {view === 'day' ? (
        <div className="space-y-3">
          ...
          {(!postsByDate.get(currentDate.toISOString().slice(0, 10)) || postsByDate.get(currentDate.toISOString().slice(0, 10))!.length === 0) && (
            <Card><p className="py-6 text-center text-sm text-slate-500">No posts on this day.</p></Card>
          )}
        </div>
      ) : (
                ...
                <div className="space-y-1">
                  {dayPosts.slice(0, 3).map((p) => (
                    <div key={p.id} draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', p.id)} onClick={() => setSelectedPost(p)} className={`cursor-move rounded border px-1.5 py-1 text-xs ${statusColors[p.status] ?? statusColors.draft}`}>
                      <p className="truncate">{p.title ?? (p.content.slice(0, 30) || 'Untitled')}</p>
      ...
      <Modal open={!!selectedPost} onClose={() => setSelectedPost(null)} title="Post Details" size="md"
        footer={selectedPost && <>
          <Button variant="outline" onClick={() => { setNewDate(selectedPost.scheduled_for ? selectedPost.scheduled_for.slice(0, 16) : ''); setShowReschedule(true); }}>Reschedule</Button>
        ...
        {selectedPost && (
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              <Badge variant={selectedPost.status === 'published' ? 'success' : selectedPost.status === 'failed' ? 'error' : 'info'}>{selectedPost.status}</Badge>
      ...
      <Modal open={showReschedule} onClose={() => setShowReschedule(false)} title="Reschedule Post" size="sm"
        footer={<><Button variant="outline" onClick={() => setShowReschedule(false)}>Cancel</Button><Button onClick={async () => {
          if (selectedPost && newDate) { await update(selectedPost.id, { scheduled_for: new Date(newDate).toISOString(), status: 'scheduled' }); push({ title: 'Post rescheduled', variant: 'success' }); setShowReschedule(false); setSelectedPost(null); }
        }}>Save</Button></>}>
        <Input label="New date and time" type="datetime-local" value={newDate} onChange={(e) => setNewDate(e.target.value)} />
      </Modal>
    </div>
  );
}
...
function CalendarPost({ post, onClick }: { post: Post; onClick: () => void }) {
  return (
    <div draggable onDragStart={(e) => e.dataTransfer.setData('text/plain', post.id)} onClick={onClick} className={`cursor-move rounded-lg border p-3 ${statusColors[post.status] ?? statusColors.draft}`}>
      <div className="flex items-center justify-between">
        <Badge variant={post.status === 'published' ? 'success' : post.status === 'failed' ? 'error' : 'info'}>{post.status}</Badge>
        <span className="text-xs text-slate-500">{post.scheduled_for ? formatDateTime(post.scheduled_for) : ''}</span>
      </div>
      <p className="mt-2 text-sm font-medium text-slate-900 dark:text-white">{post.title ?? 'Untitled'}</p>
