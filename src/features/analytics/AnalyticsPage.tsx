...
import { Badge, Card, CardSkeleton, EmptyState, ErrorState, Tabs } from '@/ui';
...
type AnalyticsTab = 'overview' | 'audience';
...
export function AnalyticsPage({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!embedded && (
        <Tabs
          tabs={[
            ...
            { id: 'audience', label: t('analytics.tab.audience') },
          ]}
          active={tab}
      ...
      {embedded || tab === 'overview' ? <AnalyticsOverview /> : <AnalyticsAudienceTab />}
    </div>
  );
}

/** The Audience tab has no dedicated data source yet — real audience
 * inference already runs inside the AI Assistant (`runAudienceInferenceAgent`
 * in assistantOrchestrator.ts). This tab is a placeholder until that data is
 * surfaced here directly, kept as a tab rather than a standalone page. */
function AnalyticsAudienceTab() {
  const { t } = useLanguage();
  return (
    <Card>
      <EmptyState
