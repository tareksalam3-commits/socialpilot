...
export function NotificationCenter() {
  ...
  // Bell sits at the end (RTL-aware) of the header, so the panel anchors to
  // the physical side that matches — 'left' in RTL, 'right' in LTR.
  const align = dir === 'rtl' ? 'left' : 'right';
  const position = useOverlayPosition(open, triggerRef, { align, direction: 'down', gap: 8 });
        ...
        createPortal(
          <div
            ...
            style={{
              ...
              top: position.top,
              left: position.left,
              transform: overlayAnchorTransform(align, 'down'),
            }}
            // Same "Dropdown/Popover/Notification" layering tier (z-40) as
            // Dropdown.tsx — portaled to document.body so the header's own
            // stacking context / any ancestor overflow can't clip it, which
            // is what let this panel get trapped behind page content before.
            className="z-40 w-[22rem] max-w-[90vw] animate-scale-in overflow-hidden rounded-xl border border-slate-200 bg-white shadow-popover dark:border-slate-800 dark:bg-slate-900"
          >
