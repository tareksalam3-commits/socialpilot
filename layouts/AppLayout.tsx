...
const navSections = [
  {
    ...
    items: [
      ...
      { to: '/app/insights', labelKey: 'nav.insights', icon: Brain },
...
export function AppLayout({ children }: { children?: ReactNode }) {
  ...
  // Pull-to-refresh (mobile only, see hook): remounting the routed content
  // under a fresh key forces every page's data hooks to re-run their fetch
  // from scratch — the same guaranteed-fresh mechanism native pull-to-refresh
  // gives you, without threading a refetch prop through every single page.
  const handleRefresh = async () => {
    setRefreshTick((v) => v + 1);
    await new Promise((resolve) => setTimeout(resolve, 450));
  };
  const { containerRef: scrollRef, pullDistance, state: pullState } = usePullToRefresh<HTMLElement>(handleRefresh);
    ...
    <>
      ...
      <div className="border-t border-slate-200 p-2 dark:border-slate-800">
        <button
          ...
          className="flex w-full items-center gap-2 rounded-lg px-2 py-2 text-start transition-colors duration-150 hover:bg-slate-100 dark:hover:bg-slate-800"
        >
        ...
        {/* Always visible, not tucked inside a menu — this used to be a
            dropdown item that rendered behind the mobile drawer's own
            backdrop (both at the same stacking layer), making it invisible
            and unclickable on mobile. A plain, permanently-visible button
            has no such layering to get wrong. */}
        <button
          onClick={handleSignOut}
          className={`mt-1 flex w-full items-center gap-2 rounded-lg px-2 py-2 text-sm font-medium text-rose-600 transition-colors duration-150 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/40 ${
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        <header
          className="relative z-30 flex h-16 shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:px-6"
        ...
        <main ref={scrollRef} className="relative flex-1 overflow-y-auto p-4 pb-24 sm:p-6 lg:pb-6">
          <PullToRefreshIndicator pullDistance={pullDistance} state={pullState} />
          {/* Keying on pathname + refreshTick replays the fade/slide-up animation
              on every route change AND forces a full remount (fresh data fetch)
              whenever pull-to-refresh fires. */}
          <div key={`${location.pathname}-${refreshTick}`} className="page-transition">
            {children ?? <Outlet />}
          </div>
        </main>
      </div>
