...
Deno.serve(async (req: Request) => {
  ...
  if (oauthError) return await redirectToApp(supabase, { platform: 'tiktok', error: oauthError });
  if (!code || !state) return await redirectToApp(supabase, { platform: 'tiktok', error: 'missing_code_or_state' });

  const creds = await getCredentials(supabase, ['tiktok_client_key', 'tiktok_client_secret']);
  if (!creds.tiktok_client_key || !creds.tiktok_client_secret) return await redirectToApp(supabase, { platform: 'tiktok', error: 'server_not_configured' });

  const consumed = await consumeOAuthState(supabase, 'tiktok', state);
  if (!consumed || !consumed.code_verifier) return await redirectToApp(supabase, { platform: 'tiktok', error: 'invalid_or_expired_state' });
  ...
  try {
    ...
    return await redirectToApp(supabase, { platform: 'tiktok', error: e instanceof Error ? e.message : 'unknown_error' });
  }
});
