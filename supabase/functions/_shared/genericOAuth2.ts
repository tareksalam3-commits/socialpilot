...
export async function startOAuth2Connect(supabase: SupabaseClient, req: Request, config: OAuth2ConnectConfig): Promise<Response> {
  try {
    return await startOAuth2ConnectInner(supabase, req, config);
  } catch (e) {
    // Safety net shared by x-oauth-connect, threads-oauth-connect, and
    // tiktok-oauth-connect: an unexpected throw here (e.g. a missing env
    // var, or a network error talking to Supabase) used to crash the
    // function with no CORS headers on the response, which the browser
    // then reports as an opaque "Failed to fetch" instead of a readable
    // error toast. Now it's always a structured JSON error instead.
    const message = e instanceof Error ? e.message : 'Unexpected error';
    console.error(`[${config.platform}-oauth-connect] unhandled error: ${message}`);
    return errorResponse(`Could not start OAuth flow: ${message}`, 500);
  }
}

async function startOAuth2ConnectInner(supabase: SupabaseClient, req: Request, config: OAuth2ConnectConfig): Promise<Response> {
