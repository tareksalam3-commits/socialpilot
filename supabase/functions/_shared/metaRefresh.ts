...
/** How far ahead of expiry we proactively refresh a Page token during the
 * cron sweep. Meta long-lived tokens last ~60 days; refreshing with 10 days
...
/** Refreshes Facebook Page tokens (and propagates the new token to any
 * linked Instagram Business account, which publishes through the same
 * Page token). Pass `accountId` to force-refresh one specific account
 * regardless of its expiry (used by the manual "Refresh Token" button);
...
export async function refreshMetaTokens(supabase: SupabaseClient, opts: { accountId?: string } = {}): Promise<MetaRefreshResult> {
  ...
  for (const page of (pages ?? []) as ConnectedAccountRow[]) {
    ...
    try {
      ...
      // Propagate to any Instagram Business account linked to this Page —
      // it publishes through the same Page access token.
