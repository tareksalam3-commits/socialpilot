...
Deno.serve(async (req: Request) => {
  ...
  if (account.platform !== 'threads') return errorResponse('Token refresh is only supported for Threads accounts on this endpoint', 400);
  ...
  const result = await refreshThreadsTokens(supabase, { accountId: account.id as string });
