...
Deno.serve(async (req: Request) => {
  ...
  if (account.platform !== 'x') return errorResponse('Token refresh is only supported for X accounts on this endpoint', 400);
  ...
  const result = await refreshXTokens(supabase, { accountId: account.id as string });
